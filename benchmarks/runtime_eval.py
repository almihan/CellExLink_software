...
from benchmark_utils import count_bioc_annotations, count_bioc_passages, write_csv
    ...
    for inp in inputs:
        ...
        if a.skip_existing and final.exists(): status="skipped_existing"; elapsed=0.0
            ...
            try:
                ...
                else: pipe.normalize_bioc(input_xml=inp, output_xml=norm_xml)
            except Exception as exc: status=f"failed: {type(exc).__name__}: {exc}"
            elapsed=time.perf_counter()-start
        rows.append({"dataset":ds,"mode":a.mode,"input_xml":str(inp),"ner_xml":str(ner_xml) if ner_xml.exists() else "","normalized_xml":str(norm_xml) if norm_xml.exists() else "","final_output":str(final),"status":status,"elapsed_seconds":elapsed,"input_passages":count_bioc_passages(inp),"input_annotations":count_bioc_annotations(inp),"output_annotations":count_bioc_annotations(final) if final.exists() else ""})
